    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-500">Manage customer orders and fulfillment</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4 shadow-sm backdrop-blur-sm transition-all hover:border-zinc-700">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-sm text-zinc-500 font-medium">Total Orders</p><p className="text-2xl font-bold text-white">{stats.total.count}</p></div>
          </div>
        </div>
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4 shadow-sm backdrop-blur-sm transition-all hover:border-zinc-700">
          <div className="flex items-center gap-3">
            <div className="rounded-lg bg-yellow-500/10 p-2"><Package className="h-5 w-5 text-yellow-400" /></div>
            <div><p className="text-sm text-zinc-500 font-medium">Processing</p><p className="text-2xl font-bold text-white">{stats.confirmed.count}</p></div>
          </div>
        </div>
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4 shadow-sm backdrop-blur-sm transition-all hover:border-zinc-700">
          <div className="flex items-center gap-3">
            <div className="rounded-lg bg-purple-500/10 p-2"><Truck className="h-5 w-5 text-purple-400" /></div>
            <div><p className="text-sm text-zinc-500 font-medium">Shipped</p><p className="text-2xl font-bold text-white">{stats.shipped.count}</p></div>
          </div>
        </div>
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4 shadow-sm backdrop-blur-sm transition-all hover:border-zinc-700">
          <div className="flex items-center gap-3">
            <div className="rounded-lg bg-green-500/10 p-2"><CheckCircle className="h-5 w-5 text-green-400" /></div>
            <div><p className="text-sm text-zinc-500 font-medium">Delivered</p><p className="text-2xl font-bold text-white">${stats.delivered.amount.toLocaleString()}</p></div>
          </div>
        </div>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-12 text-center shadow-sm backdrop-blur-sm">
          <ShoppingCart className="mx-auto h-12 w-12 text-zinc-700 opacity-50" />
          <h3 className="mt-4 text-lg font-semibold text-white">No sales orders yet</h3>
          <p className="mt-2 text-zinc-500">Create your first sales order to start processing customer orders.</p>
