...
interface LineItem { description: string; sku: string; quantity: number; unitPrice: number; taxRate: number }

export function CreatePOButton({ vendors }: { vendors: Vendor[] }) {
  const [isOpen, setIsOpen] = useState(false)
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState('')
  const [lineItems, setLineItems] = useState<LineItem[]>([{ description: '', sku: '', quantity: 1, unitPrice: 0, taxRate: 0 }])
  const router = useRouter()

  const addLine = () => setLineItems([...lineItems, { description: '', sku: '', quantity: 1, unitPrice: 0, taxRate: 0 }])
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setIsOpen(false)
      setLineItems([{ description: '', sku: '', quantity: 1, unitPrice: 0, taxRate: 0 }])
      router.refresh()
    } catch (err) {
  ...
  return (
    <>
      <button onClick={() => setIsOpen(true)} disabled={vendors.length === 0} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition-colors disabled:opacity-50">
