...
import { createAccount, seedDefaultAccounts } from '@/app/actions/accounts'
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await createAccount({
        ...
        accountType: formData.get('accountType') as 'ASSET' | 'LIABILITY' | 'EQUITY' | 'REVENUE' | 'EXPENSE',
...
export function SeedAccountsButton() {
  ...
  return (
    <button
      onClick={handleSeed}
      disabled={isLoading}
      className="inline-flex items-center gap-2 rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-700 transition-colors disabled:opacity-50"
    >
      <Sparkles className="h-4 w-4" />
      {isLoading ? 'Seeding...' : 'Seed Default Accounts'}
    </button>
  )
}
