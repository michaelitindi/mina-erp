...
export interface TaxConfig {
  ...
  complianceProvider: 'KRA_ETIMS' | 'AVALARA' | 'HMRC_MTD' | 'SARS_VAT' | 'FTA_ZATCA' | 'FIRS_VAT' | 'GSTIN_INDIA' | 'ATO_PEPPOL' | 'SEPA_EU' | 'NFE_BRAZIL' | 'CFDI_MEXICO' | 'INVOICENOW_SG' | 'TICKETBAI_SPAIN' | 'JAPAN_INVOICE' | 'CHORUS_FRANCE' | 'SDI_ITALY' | 'ETA_EGYPT' | 'EFAKTUR_INDONESIA' | 'ZATCA_SAUDI' | 'GIB_TURKEY' | 'AFIP_ARGENTINA' | 'NTS_KOREA' | 'SII_CHILE' | 'BIR_PHILIPPINES' | 'LHDN_MALAYSIA' | 'GDT_VIETNAM' | 'ETAX_THAILAND' | 'FBR_PAKISTAN' | 'KSEF_POLAND' | 'STANDARD'
